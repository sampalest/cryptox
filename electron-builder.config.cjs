const mac = {
    ...
    // Appearance-aware app icon. Assets.car is compiled from the Icon
    ...
    // electron-builder's fileAssociations only emit CFBundleTypeExtensions,
...
const nsis = {
    ...
    // No artifactName override: the installer inherits the top-level template
    // (Lockasaur-<arch>.exe). The arch stays so the x64 and arm64 installers
...
module.exports = {
    ...
    // Build AppImages with the static (type2, runtime 20251108) launcher
    ...
    // of the file name: it stays in each package's own metadata (macOS
    // Info.plist CFBundleShortVersionString, the deb control Version field, the
    // Windows exe PE version resource), so downloads read as
    // Lockasaur-<arch>.<ext>.
    ...
    // .dino is the extension Lockasaur writes; the legacy .ctx
    ...
    // One shared fuse block on all three platforms. runAsNode, the
