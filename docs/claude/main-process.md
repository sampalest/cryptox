...
- `app:set-icon`: sets the macOS Dock icon to one of the bundled appearance variants (CTX-17). The renderer sends an id validated by `normalizeAppIconId`; the handler resolves it against `dist/appicons/<id>.png` (`public/appicons` in dev, mirroring how `index.html` is located) and calls `app.dock.setIcon`. `"default"` resets with `app.dock.setIcon(null)` so the bundle's `Assets.car` keeps the Dock icon appearance-aware on macOS 26+ instead of freezing one static image. Returns `true` on success and an inert `false` for an untrusted sender, an unknown id, a non-macOS platform, or an unreadable image; the catch logs a fixed string (the renderer-supplied value stays out of the log).
...
- `normalizeAppIconId(value)`: allowlist for the Dock icon id (`default`, `dark`, `clear-light`, `clear-dark`, `tinted-light`, `tinted-dark`, matching the PNGs `scripts/generate-appicon.mjs` writes to `public/appicons`); anything else returns `null` (the handler then answers with an inert `false`). The id is resolved to a bundled file name by the handler, so path fragments must never survive normalization.
