...
- `runSmokeTest()`: only under `LOCKASAUR_SMOKE_TEST`. Verifies `window.lockasaur` exists in the renderer (which proves the preload bridge works under `sandbox: true`), that `window.open` does not create a second window, and that `app:set-icon` accepts an allowlisted id (true on macOS, inert false elsewhere) while rejecting a non-allowlisted one, then exits 0/1; used by `tests/e2e/smoke.js`.
