...
- `WINDOW_DESIGN_SIZE` / `WINDOW_SIZE_PRESETS`: the 700x660 design size and the fixed presets the renderer may pick by allowlisted id (`default` zoom 1, `l` zoom 1.08, `xl` zoom 1.25). Each preset is only a zoom factor: the OS window scales while `setZoomFactor` keeps the CSS viewport at the design size, so every preset is the same layout scaled proportionally. No dimensions ever cross the IPC boundary.
- `appliedWindowSize`: the preset id last accepted by `applyWindowSize`; a recreated window (macOS dock activate) comes back at that size and zoom without waiting for the renderer to reapply it.
...
- `windowBoundsForPreset(preset)`: OS window bounds for a preset: the design size plus, on Win/Linux, the `FRAMELESS_GUTTER` on every side, all multiplied by the preset's zoom (the gutter lives in the CSS viewport, so it scales like everything else) and rounded.
- `applyWindowSize(id)`: applies a preset to the live window. Refuses (returns `false`) when the target bounds exceed the `workArea` of the display containing the window, so an oversized preset is unavailable rather than broken (the renderer store rolls the selection back). Otherwise resizes around the window's current center clamped into the work area, in this order: widen the min/max limits, `setBounds`, re-pin min == max to the new bounds, `setResizable(false)` / `setMaximizable(false)` / `setFullScreenable(false)`, then `setZoomFactor`. Records `appliedWindowSize` and returns `true`.
- `showWindowWhenReady()`: shows the window if it still exists and is not visible yet. The window is created with `show: false` (CTX-17) so launch never flashes an empty transparent frame; this helper is wired to the first of three signals: `ready-to-show` (normal path), the `files:renderer-ready` handler (fallback, since `ready-to-show` has a history of firing late for transparent frameless windows on Windows/Linux), and a 2000 ms safety timer created in `createWindow()` (cleared on `closed`) so a broken renderer never leaves an invisible app.
- `createWindow()`: creates the fixed-size window at the applied preset (700x660 of visible app at `default`; 660 = the design's 618px content area plus the 42px in-app titlebar, so the tallest screen keeps top/bottom margin without clipping). The size is pinned three ways: `resizable: false`, `maximizable: false` plus `fullscreenable: false`, and min/max sizes equal to the initial bounds (min == max backs the resizable flag with WM size hints, which some Linux window managers require). `webPreferences.zoomFactor` starts at the applied preset's zoom, and a `did-finish-load` handler re-asserts that zoom on every load: Chromium persists per-origin zoom in the profile, which could otherwise disagree with the applied preset (e.g. a saved larger preset that no longer fits the display). Created with `contextIsolation: true`, `nodeIntegration: false`, `sandbox: true`, `webSecurity: true` and the bundled preload. The window starts hidden (`show: false`) and becomes visible via `showWindowWhenReady()` (see above). The window is transparent on every platform (APP-12): the visible frame is the CSS-rounded `#app`. macOS keeps native traffic lights via `titleBarStyle: "hidden"` + `trafficLightPosition` (positioned inside the renderer's 42px titlebar); Windows/Linux are fully frameless (`frame: false`) with custom minimize/close controls in the renderer, `hasShadow: false` (the window shadow is the CSS one on `#app.platform-frameless`, so the OS must not trace the rectangular transparent bounds), and window bounds enlarged by `FRAMELESS_GUTTER` (32px, shared/constants.js) on every side so that CSS shadow paints without being clipped at the window edge; the visible app stays 700x660. Registers `setWindowOpenHandler` (denies every renderer-initiated window) and a `will-navigate` guard that only allows the dev server origin (dev) or the bundled `dist/index.html` file URL (prod); blocked navigations log a fixed string. Loads `VITE_DEV_SERVER_URL` in dev (opens devtools unless `IS_TEST`), otherwise `dist/index.html`. The renderer CSP lives as a meta tag in `index.html` (CTX-12).
...
- `runSmokeTest()`: only under `LOCKASAUR_SMOKE_TEST`. Verifies `window.lockasaur` exists in the renderer (which proves the preload bridge works under `sandbox: true`), that `window.open` does not create a second window, and that `app:set-icon` accepts an allowlisted id (true on macOS, inert false elsewhere) while rejecting a non-allowlisted one, that the window content size matches the design size plus the frameless gutter, and that `window:set-size` accepts the (size-preserving) `default` preset while rejecting a non-allowlisted id, then exits 0/1; used by `tests/e2e/smoke.js`.
...
- `files:renderer-ready`: `isTrustedSender`, then marks the renderer ready, shows the window if it is still hidden (`showWindowWhenReady`), flushes queued open-file events, triggers the smoke test when enabled.
- `app:info`: returns `{ locale, name, platform }` (or `undefined` for an untrusted sender).
- `app:set-icon`: sets the macOS Dock icon to one of the bundled appearance variants or the `Locked` padlock-dino variant (CTX-17). The renderer sends an id validated by `normalizeAppIconId`; the handler resolves it against `dist/appicons/` (`public/appicons` in dev, mirroring how `index.html` is located) and builds one multi-representation `NativeImage` from `<id>.png` (512 px, scale factor 1) plus `<id>@2x.png` (1024 px, scale factor 2, skipped if absent), then calls `app.dock.setIcon`. The hand-built pair exists because Electron's `nativeImage` cannot decode `.icns`, so this is the closest icns equivalent: macOS picks the representation matching the display scale instead of upscaling one bitmap. `"default"` resets with `app.dock.setIcon(null)` so the bundle's `Assets.car` keeps the Dock icon appearance-aware on macOS 26+ instead of freezing one static image. Returns `true` on success and an inert `false` for an untrusted sender, an unknown id, a non-macOS platform, or when no variant file could be read; the catch logs a fixed string (the renderer-supplied value stays out of the log).
...
- `window:minimize` / `window:close`: trust check, then minimize or close the window.
- `window:set-size`: trust check, `normalizeWindowSizeId`, then `applyWindowSize` (see above). Returns `true` when the preset was applied, an inert `false` for an untrusted sender, an unknown id, or a preset the current display cannot hold; the catch logs a fixed string (the renderer-supplied value stays out of the log).
...
- `files:confirm-delete-encrypted`: `validateDeletePath` (must end in `.dino` or the legacy `.ctx`), native confirm dialog defaulting to Keep, removes the file only on explicit Delete. Deletion goes to the system Trash via `shell.trashItem` (the native, iCloud/permission-aware path, and safe because the file is ciphertext), falling back to a permanent `fs.promises.unlink` where Trash is unavailable. Returns `{ deleted: <bool>, error?: true }`: `error: true` when both Trash and unlink fail (e.g. a file macOS itself cannot remove, which Finder surfaces as error -8058), so the renderer can tell the user instead of silently leaving it in place.
...
- `normalizeAppIconId(value)`: allowlist for the Dock icon id (`default`, `dark`, `clear-light`, `clear-dark`, `tinted-light`, `tinted-dark`, `locked`; the first six are the PNGs `scripts/generate-appicon.mjs` writes to `public/appicons`, `locked` the padlock-dino `scripts/generate-icons.mjs` writes there from `build/icon.svg`); anything else returns `null` (the handler then answers with an inert `false`). The id is resolved to a bundled file name by the handler, so path fragments must never survive normalization.
- `normalizeWindowSizeId(value)`: allowlist for the window-size preset id (`default`, `l`, `xl`); anything else returns `null` (the handler then answers with an inert `false`). Ids resolve to main-process presets, so raw dimensions never cross the bridge.
...
- `FRAMELESS_GUTTER` (32): transparent gutter in px reserved around `#app` on the frameless platforms (win32/linux) so the CSS window shadow is not clipped at the window bounds; `createWindow()` enlarges the window by it, and the value is mirrored in `sass/components/master.scss` (`#app.platform-frameless`).
