...
The config carries a `win` block (nsis + portable, `build/icon.ico`), a `linux` block (AppImage + deb, the `build/icons/` PNG set), and an `nsis` block (per-user install with no elevation, desktop + start-menu shortcuts). `fileAssociations` registers the `.ctx` extension on all platforms and adds `mimeType: application/x-cryptox` for Linux mime registration. Windows and Linux ship unsigned for the alpha too (Windows shows a SmartScreen prompt). `build/icon.ico` and `build/icons/*.png` are committed and regenerated from `public/icon.png` by `npm run electron:generate-icons` (macOS-only, uses `sips` plus an inline ICO packer, `scripts/generate-icons.mjs`); the `.icns` files are committed separately. Each platform is built for both x64 and arm64 on native-architecture CI runners; `artifactName` carries `${arch}` so the six artifact sets stay distinct in one release (a local `electron:build` still produces just the host arch).
