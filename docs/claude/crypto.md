...
- `validateKdfParams(meta)`: kdf must be argon2id, salt must decode to 16 bytes, opslimit in [1, 4], memlimit in [8192, 1 GiB], keyLen 32. Bounds are hardcoded libsodium MIN..SENSITIVE presets (sodium constants need async init; this module stays pure). Rejects crafted headers demanding gigabytes (KDF DoS).
...
- `isDirectory(path)`: lstat-based check.
- `rmRf(path)`: recursive removal, tolerant of missing paths.
- `getFilesDir(dir)`: promisified readdir (legacy helper).
