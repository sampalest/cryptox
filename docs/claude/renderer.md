...
- `DinoLogo.vue`: the mascot (`assets/dino-vector.svg` as `<img>`), `ringPulse` halo, `logoShadow` CSS drop-shadow animation, click/keyboard replays `bounceInFwd` (timeout cleared on unmount). Prop `size`.
- `LockasaurLock.vue`: the padlock artwork, inlined via `assets/lockasaur-open.svg?raw` + `v-html` (static build-time content, needed so CSS can reach the `#Arco` shackle group). Prop `lockState`: `open` | `locked` (arc translated down into the body) | `locking` (arc slides shut, `arcoClose`) | `unlocking` (arc springs open, `arcoOpen`). Transform-only animation on `#Arco` with `transform-box: fill-box`.

## src/renderer/router/index.js

Single route on hash history: `/` -> `Home.vue`. (The `/about` route and `About.vue` were replaced by the About overlay.)
...
- `store/ui.js`: `aboutOpen` / `settingsOpen` booleans with `openAbout` / `openSettings` (mutually exclusive) / `closeOverlays`.
...
The drag-and-drop overlay, now a centered glass sheet (dashed accent border, floating files icon). Prevents default on all drag events, toggles its own visibility while a drag is over the window (opacity 0 but interactive when idle, so clicks bubble through to the logo behind), and emits `imageFile` with `e.dataTransfer.files` (DOM FileList) on drop. Name and emit event are historical ("image-loader"/"imageFile"); it handles any file.
