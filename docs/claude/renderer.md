...
Root shell: fake navbar strip, app title, `<router-view>`. No locale plumbing (the unused `messages.js` i18n table was removed; user-facing strings are consolidated in `filecryto.js`'s `FAILURE_MESSAGES`).
