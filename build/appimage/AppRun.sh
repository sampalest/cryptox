#!/bin/sh
# Cryptox AppImage launcher (APP-11). Ubuntu 24.04+ restricts unprivileged
...
# pkexec-authorized install of an AppArmor profile scoped to the Cryptox
...
PROFILE_PATH="/etc/apparmor.d/cryptox-appimage"
...
USERNS_PROC="${CRYPTOX_USERNS_PROC:-/proc/sys/kernel/apparmor_restrict_unprivileged_userns}"
...
profile cryptox-appimage /tmp/.mount_Crypt*/cryptox flags=(unconfined) {
  userns,

  include if exists <local/cryptox-appimage>
}'

print_manual_hint() {
    printf '%s\n' "Cryptox: this system restricts unprivileged user namespaces (Ubuntu 24.04+), so the AppImage cannot start its sandbox without a one-time AppArmor profile." >&2
...
consent=no
if [ -t 0 ] && [ -t 2 ]; then
    printf '%s\n' "Cryptox needs a one-time AppArmor profile to run sandboxed on this system." >&2
    ...
    if zenity --question --title "Cryptox" --text "Cryptox needs a one-time system permission (an AppArmor profile) to run with its security sandbox on this system. Install it now? You will be asked for your password."; then
        consent=yes
    fi
else
...
if pkexec /bin/sh -c 'install -D -m 0644 -o root -g root "$1" /etc/apparmor.d/cryptox-appimage && apparmor_parser -r /etc/apparmor.d/cryptox-appimage' sh "$tmpdir/profile"; then
    exec "$REAL" "$@"
fi
