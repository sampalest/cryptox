...
| Fixed | On Windows and Linux the drag-and-drop overlay showed no blur (transparent windows never composite `backdrop-filter` there, an upstream Chromium limitation): while a drag is over the window the content behind the drop zone is now blurred directly, and the titlebar, overlays, buttons and inputs use more opaque frosted fills on those platforms | - |
