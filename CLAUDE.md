...
- [ipcValidation.js](src/main/ipcValidation.js): the single gate every IPC handler passes through. Sender trust check, payload normalization, operation-id pattern, delete-path allowlist, external-URL allowlist, app-icon id allowlist.
...
- `store/`: Pinia stores: [files.js](src/renderer/store/files.js) (selected files), [theme.js](src/renderer/store/theme.js) (light/dark/system appearance), [ui.js](src/renderer/store/ui.js) (overlay visibility), [appIcon.js](src/renderer/store/appIcon.js) (macOS Dock icon choice).
...
- `files:confirm-delete-encrypted` deletes only paths ending in the `.dino` extension (or the legacy `.ctx`) and always behind a native confirm dialog; the removal goes to the system Trash (`shell.trashItem`, falling back to a permanent `unlink` where Trash is unavailable), acceptable precisely because the file is ciphertext (unlike the plaintext original, which must never be trashed). `files:confirm-delete-original` (the post-encrypt prompt) deletes only a source path the main process itself recorded from a successfully completed encrypt in the current session; each recorded path is consumed by a single prompt, the dialog defaults to Keep, a symlink swapped in at the recorded path after encryption is refused at delete time (re-lstat), and deletion is permanent (`fs.rm`, not the system trash, so plaintext never lingers in the trash). `shell:open-external` opens only https URLs from the hardcoded allowlist in ipcValidation.js. `app:set-icon` accepts only allowlisted icon ids (never renderer-supplied paths) and resolves them against the bundled `appicons` directory. `window:set-size` accepts only allowlisted preset ids (never renderer-supplied dimensions) and resolves them against the main process's own preset map; the window stays non-resizable (resizable false, min == max size hints) at every preset.
